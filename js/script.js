        ...
        let pres = JSON.parse(localStorage.getItem("presencas") || "[]");

        function login() { if (user.value == "admin" && pass.value == "123") { loginSistema.classList.add("hidden"); painelSistema.classList.remove("hidden"); lista() } else alert("erro") }
        function cadastrar() { alunos.push({ nome: nome.value, idade: idade.value, faixa: faixa.value }); localStorage.setItem("alunos", JSON.stringify(alunos)); lista() }
        function lista() {
            let h = "<table><tr><th>Nome</th><th>Faixa</th></tr>";
            let s = "";

            alunos.forEach(a => {
                h += `<tr><td>${a.nome}</td><td>${a.faixa}</td></tr>`;
                s += `<option>${a.nome}</option>`;
            });

            h += "</table>";

            document.getElementById("lista").innerHTML = h;
            document.getElementById("alunoPresenca").innerHTML = s;
        }
        function presenca() { pres.push({ nome: alunoPresenca.value, data: dataPresenca.value }); localStorage.setItem("presencas", JSON.stringify(pres)); alert("ok") }
        function relatorio() { let m = mes.value; let c = {}; pres.filter(p => p.data.startsWith(m)).forEach(p => c[p.nome] = (c[p.nome] || 0) + 1); let h = ""; for (let n in c) h += `<p>${n}:${c[n]}</p>`; document.getElementById("relatorio").innerHTML = h }


        function trocarVideo(id) {
            document.getElementById("playerVideo").src = "https://www.youtube.com/embed/" + id;
        }
